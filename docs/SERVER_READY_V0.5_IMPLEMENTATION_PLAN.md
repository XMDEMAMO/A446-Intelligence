...
| 阶段 G 验证与发布准备 | 进行中（8/9 门禁通过） | 2026-09-23：完整本地门禁、一次性真实服务栈浏览器门禁、目标 HTTPS/WSS、成对备份与隔离恢复、跨 schema 版本回退、候选恢复以及 systemd Worker 异常重启和同 Agent 重连均已通过。预发布运行候选为 `ef397b69b23a68b2e7d139b6e0518325578c2666`；临时 Worker 凭据已撤销。当前只剩两台已登录设备上的真实 Codex/Antigravity 角色闭环与恢复验收。详细记录见 `docs/V0.5_RELEASE_ACCEPTANCE_REPORT.md`。 |
