...
A446 Intelligence 是一个通用的本地与分布式 AI Agent 任务控制平台。当前阶段提供 N 台个人设备的私人局域网协作预览，而不是生产级云服务。

当前仓库已经包含可运行的最小原型：

- apps/web：React + TypeScript 的 Agent 群聊控制台；一个任务对应一个群聊，显示角色简报、成果附件和账号用量。
- apps/agent-hub：local-agent-hub v0.5.0-alpha.4，共享编排核心负责规划/执行/审核闭环、Attempt/Lease、局域网本地成果传输、运行期资源刷新、动态 Agent/模型调度、Token/额度统计和本地 Worker 通信。
...
多设备私人局域网测试请直接双击根目录的 `START-A446-MULTI-DEVICE-LAN.bat`。协调设备同时运行 Hub、网页和本机 Agent，其余设备自动发现本机已登录 Provider 并连接，详细说明见 [A446 多设备局域网版使用说明](MULTI-DEVICE-LAN-README.md)。旧的固定双机入口 `START-A446-PRIVATE-LAN.bat` 继续保留，便于复现 r8。
